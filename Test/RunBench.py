...
the final committed count. This measures neither balance execution nor consensus.
"""
import argparse
import json
...
import time
import psutil
from NetworkTest import FLAGS, Run, GetStats, StartNode, StopNode, SendFrame


def RunNode(binary, work, dataset, count, threads, sync, batch, repeat, block_ms=50):
    ...
    sender = None
    try:
        sender = subprocess.Popen([str(binary/'sender.exe'), '--file', str(dataset),
...
def RunLatency(binary, work, dataset, block_ms, count=50):
...
def RunReplica(binary, work, dataset, count, repeat):
    nodes = []
    try:
        ...
        while time.perf_counter()-started < 80:
            ...
            if first['committed'] == count and second['committed'] == count and first['head'] == second['head']:
                ...
                return {'repeat': repeat, 'count': count, 'sync': 1, 'seconds': elapsed,
                        ...
                        'average_cpu_cores': [(sum(p.cpu_times()[:2])-start)/elapsed for p,start in zip(processes,cpu_start)],
                        'sender': sent}
            time.sleep(.02)
...
def Main():
    ...
    if args.suite == 'replica':
        for repeat in range(args.repeats):
            Save('replica', RunReplica(binary, work, dataset, args.count, repeat))
        return
    if args.suite == 'all':
        for repeat in range(args.repeats):
    ...
    for repeat in range(args.repeats):
        for threads in args.threads:
            Save('node', RunNode(binary, work, dataset, args.count, threads, 1, 128, repeat))
        if 8 in args.threads:
            Save('node', RunNode(binary, work, dataset, args.count, 8, 0, 128, repeat))
            Save('node', RunNode(binary, work, dataset, args.count, 8, 1, 1, repeat))
    for block_ms in (10, 50):
        Save('latency', RunLatency(binary, work, dataset, block_ms))
